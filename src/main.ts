...
class TractiveNext extends utils.Adapter {
  ...
  private async authenticate(): Promise<void> {
    ...
    if (!this.accessToken || !this.userId) {
      throw new Error(
        "Authentication response did not contain access_token and user_id.",
      );
    }

    this.log.debug(
      `Authenticated as Tractive user ${this.userId}; token expires at ${this.expiresAt}.`,
    );
  }
  ...
  private async apiGet<T>(url: string, retry = true): Promise<T> {
    try {
      ...
      if (retry && (status === 401 || status === 403)) {
        this.log.warn(
          `Tractive returned HTTP ${status}; refreshing token and retrying once.`,
        );
        await this.authenticate();
  ...
  private async poll(): Promise<void> {
    ...
    try {
      if (
        !this.accessToken ||
        (this.expiresAt > 0 &&
          this.expiresAt <= Math.floor(Date.now() / 1000) + 60)
      ) {
        await this.authenticate();
      }

      const trackers = await this.apiGet<ApiRecord[]>(
        `/user/${this.userId}/trackers`,
      );
      ...
      for (const tracker of trackers) {
        ...
        await this.ensureObject(
          trackerId,
          "device",
          this.asString(tracker.name) || trackerId,
        );
  ...
  private async fetchSection(
    ...
    try {
      ...
      const apiCode = (
        axiosError?.response?.data as { code?: number } | undefined
      )?.code;
  ...
  private async writeOsmMapLink(
    ...
    await this.ensureState(
      `${trackerId}.device_pos_report.latitude`,
      "latitude",
      {
        value: lat,
        type: "number",
    ...
    await this.ensureState(
      `${trackerId}.device_pos_report.longitude`,
      "longitude",
      {
        value: lon,
        type: "number",
  ...
  private async writeOverview(
    ...
    const batteryLevel = Number(
      hw?.battery_level ?? tracker?.battery_level ?? NaN,
    );
    const sensorUsed = this.asString(pos?.sensor_used);
    const batteryState = this.asString(
      tracker?.battery_state ?? hw?.battery_state,
    );
  ...
  private isUnixTimestampField(key: string, value: number): boolean {
    ...
    const keyHint =
      /(?:^|_)(time|timestamp|date|expires|created|updated|last_seen|time_pos|time_rcvd)(?:_|$)/i.test(
        key,
      ) ||
      /_(at|ts)$/i.test(key) ||
  ...
  private async ensureState(
    ...
    if (typeChanged || roleChanged) {
      ...
      this.log.info(
        `Updated inferred type/role of ${id} to ${normalized.type}/${normalized.role}.`,
      );
    }
  }
